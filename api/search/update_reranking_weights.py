...
def update_reranking_weights():
    """
    후보 피드백 데이터를 집계하여 재정렬에 사용할 가중치(벡터, BM25, LLM bonus, 피드백)를 업데이트합니다.
    집계 결과에 따라 다른 A/B Variant를 적용할 수도 있습니다.
    """
    feedback_col = db["candidate_feedback"]
    config_col = db["search_config"]

    # 예시: 모든 후보의 평균 CTR을 계산 (CTR = clicks / (impressions + 1))
    pipeline = [
        {
            "$group": {
                "_id": None,
    ...
    if result:
        ...
        # A/B Variant 선택: CTR 값에 따라 다른 가중치 세트를 적용하는 예시
        if avg_ctr > 0.1:
